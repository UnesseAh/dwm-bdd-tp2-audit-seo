...
// Validate allowed languages to prevent layout breaks
if (!in_array($lang, ['ar', 'fr', 'en'])) {
    $lang = 'fr';
}

// Simple translation array for the navigation links
$ui = [
...
// Fetch only cosmetics items from the database
$stmt = $pdo->prepare("SELECT * FROM products WHERE category = 'cosmetics'");
$stmt->execute();
...
<head>
    ...
    <meta name="description" content="Découvrez les cosmétiques naturels marocains comme l’huile d’argan, le savon noir et le ghassoul, issus des traditions du Maroc.">
    <style>
        body {
